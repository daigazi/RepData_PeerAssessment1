...
## 1What is mean total number of steps taken per day?
...
### 3.1. the total number of rows with NAs  
the total number of rows with NAs`r nrow(dat)-nrow(tmp)`
```{r}
nrow(dat)-nrow(tmp)
```

### 3.2.  filling in all of the missing values in the dataset  
```{r}
...
### 4.1 .Create a new factor variable in the dataset with two levels – “weekday” and “weekend” i
```{r,message=FALSE}
library(lubridate)
